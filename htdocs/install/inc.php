...
if (!defined('ADODB_PATH')) {
	$foundpath = DOL_DOCUMENT_ROOT.'/embarquements/adodbtime/';
	if (!is_dir($foundpath)) {
...
function pHeader($subtitle, $next, $action = 'set', $param = '', $forcejqueryurl = '', $csstable = 'main-inside')
{
	...
	if ($jQueryUiCustomPath) {
		...
		print '<link rel="stylesheet" type="text/css" href="../embarquements/jquery/css/'.$jquerytheme.'/jquery-ui.min.css" />'."\n"; // JQuery
	}
	...
	if ($jQueryCustomPath) {
		...
		print '<script type="text/javascript" src="../embarquements/jquery/js/jquery.min.js"></script>'."\n";
	}
	if ($jQueryUiCustomPath) {
		print '<script type="text/javascript" src="'.$jQueryUiCustomPath.'jquery-ui.min.js"></script>'."\n";
	} else {
		print '<script type="text/javascript" src="../embarquements/jquery/js/jquery-ui.min.js"></script>'."\n";
	}
