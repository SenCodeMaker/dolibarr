...
class CommentParser
{
    ...
    private function parseEmbeddedData($subject)
    {
        ...
        while (preg_match('/{@(\w+)\s?([^}]*)}/ms', $subject, $matches)) {
            ...
            $subject = str_replace($matches[0], '', $subject);
            if ($name == 'pattern') {
                throw new Exception('Inline pattern tag should follow {@pattern /REGEX_PATTERN_HERE/} format and can optionally inclusions PCRE modifiers following the ending `/`');
