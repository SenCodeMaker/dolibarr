...
class JavascriptRenderer
{
    ...
    protected $inclusionsVendors = true;
    ...
    public function setOptions(array $options)
    {
        ...
        if (array_key_exists('inclusions_vendors', $options)) {
            $this->setinclusionsVendors($options['inclusions_vendors']);
        }
    ...
    /**
     * Whether to inclusions vendor assets
     *
     * You can only inclusions js or css vendors using
     * setinclusionsVendors('css') or setinclusionsVendors('js')
     *
     * @param boolean $enabled
     */
    public function setinclusionsVendors($enabled = true)
    {
        if (is_string($enabled)) {
            $enabled = array($enabled);
        }
        $this->inclusionsVendors = $enabled;

        if (!$enabled || (is_array($enabled) && !in_array('js', $enabled))) {
            // no need to call jQuery.noConflict() if we do not inclusions our own version
    ...
    /**
     * Checks if vendors assets are inclusionsd
     *
     * @return boolean
     */
    public function areVendorsinclusionsd()
    {
        return $this->inclusionsVendors !== false;
    }
     ...
     *
     * This does not inclusions controls provided by collectors
     *
     * @return array
     */
    ...
    public function getAssets($type = null, $relativeTo = self::RELATIVE_PATH)
    {
        ...
        if ($this->inclusionsVendors !== false) {
            if ($this->inclusionsVendors === true || in_array('css', $this->inclusionsVendors)) {
                $cssFiles = array_merge($this->cssVendors, $cssFiles);
            }
            if ($this->inclusionsVendors === true || in_array('js', $this->inclusionsVendors)) {
    ...
    /**
     * Renders the html to inclusions needed assets
     *
