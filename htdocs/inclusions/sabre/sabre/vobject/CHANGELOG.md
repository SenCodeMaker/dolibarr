...
* #284: When generating `CANCEL` iTip messages, we now inclusions `DTEND`.
  (@kewisch)
...
* #250: Recurring all-day events are incorrectly inclusionsd in time range
...
* #142: `CANCEL` and `REPLY` messages now inclusions the `DTSTART` from the
  original event.
...
* Removed: CommaSeparatedText property. Now inclusionsd into Text.
