  ...
  var Vim = function() { return vimApi; } //{
    ...
    var operators = {
      ...
      'delete': function(cm, args, ranges) {
        ...
        var inclusionsLineBreak = vim.insertMode
        return clipCursorToContent(cm, finalHead, inclusionsLineBreak);
      },
    ...
    function clipCursorToContent(cm, cur, inclusionsLineBreak) {
      var line = Math.min(Math.max(cm.firstLine(), cur.line), cm.lastLine() );
      var maxCh = lineLength(cm, line) - 1;
      maxCh = (inclusionsLineBreak) ? maxCh + 1 : maxCh;
    ...
    function charIdxInLine(start, line, character, forward, inclusionsChar) {
      var idx;
      if (forward) {
        idx = line.indexOf(character, start + 1);
        if (idx != -1 && !inclusionsChar) {
          idx -= 1;
        }
      } else {
