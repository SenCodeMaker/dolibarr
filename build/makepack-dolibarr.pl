...
if ($nboftargetok) {
	...
	if ($CHOOSEDTARGET{'-CHKSUM'})
	{
		...
		$ret=`rm -f  $SOURCE/htdocs/embarquements/autoload.php`;
	...
	if ($nboftargetneedbuildroot)
	{
		...
		# Removed files installed by the awful composer   	    
   	    $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/geoip/sample*.*`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/bin`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/ckeditor/ckeditor/adapters`;		# Keep this removal in case we embed libraries
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/ckeditor/ckeditor/samples`;		# Keep this removal in case we embed libraries
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/ckeditor/_source`;					# _source must be kept into tarball for official debian, not for the rest
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/composer`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/doctrine`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/jquery/plugins/multiselect/MIT-LICENSE.txt`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/jquery/plugins/select2/release.sh`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/mike42/escpos-php/doc`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/mike42/escpos-php/example`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/mike42/escpos-php/test`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/mobiledetect/mobiledetectlib/.gitmodules`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/mobiledetect/mobiledetectlib/docs`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/nnnick/chartjs/.github`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/nnnick/chartjs/docs`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/nnnick/chartjs/samples`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/nnnick/chartjs/scripts`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/nnnick/chartjs/src`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/nnnick/chartjs/test`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/nusoap/lib/Mail`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/nusoap/samples`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/php-iban/docs`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/sabre/sabre/*/tests`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/stripe/tests`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/stripe/LICENSE`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/tecnickcom/examples`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/tecnickcom/tcpdf/fonts/dejavu-fonts-ttf-*`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/tecnickcom/tcpdf/fonts/freefont-*`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/tecnickcom/tcpdf/fonts/ae_fonts_*`;
        $ret=`rm -fr $BUILDROOT/$PROJECT/htdocs/embarquements/tecnickcom/tcpdf/fonts/utils`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/tecnickcom/tcpdf/tools`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/vendor`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/webmozart`;
        $ret=`rm -f  $BUILDROOT/$PROJECT/htdocs/embarquements/autoload.php`;
	}
	...
	foreach my $target (sort keys %CHOOSEDTARGET) 
	{
		...
		if ($target eq 'TGZ') 
		{
			...
			$ret=`rm -fr $BUILDROOT/$FILENAMETGZ/htdocs/embarquements/ckeditor/_source`;	# We can't remove it with exclude file, we need it for some tarball packages
		...
		if ($target eq 'XZ') 
		{
			...
			$ret=`rm -fr $BUILDROOT/$FILENAMEXZ/htdocs/embarquements/ckeditor/_source`;	# We can't remove it with exclude file, we need it for some tarball packages
		...
		if ($target eq 'ZIP') 
		{
			...
			$ret=`rm -fr $BUILDROOT/$FILENAMEZIP/htdocs/embarquements/ckeditor/_source`;	# We can't remove it with exclude file, we need it for some tarball packages
		...
		if ($target =~ /RPM/)	                 # Linux only 
		{
			...
			#$ret=`rm -fr $BUILDROOT/$FILENAMETGZ2/htdocs/embarquements/ckeditor/_source`;
		...
		if ($target eq 'DEB') 
		{
			...
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/ckeditor/ckeditor/_source/LICENSE.md`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/ckeditor/ckeditor/_source/plugins/scayt/LICENSE.md`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/ckeditor/ckeditor/_source/plugins/wsc/LICENSE.md`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/ckeditor/ckeditor/LICENSE.md`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/ckeditor/ckeditor/plugins/scayt/LICENSE.md`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/ckeditor/ckeditor/plugins/wsc/LICENSE.md`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/php-iban/LICENSE`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/jquery/plugins/flot/LICENSE.txt`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/jquery/plugins/datatables/extensions/ColReorder/License.txt`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/jquery/plugins/datatables/extensions/ColVis/License.txt`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/jquery/plugins/datatables/extensions/FixedColumns/License.txt`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/jquery/plugins/datatables/extensions/Responsive/License.txt`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/jquery/plugins/datatables/license.txt`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/jquery/plugins/select2/LICENSE`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/mike42/escpos-php/LICENSE.md`;
			$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/mobiledetect/mobiledetectlib/LICENSE.txt`;
			
			# Removed files we don't need (already removed)
			#$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/embarquements/ckeditor/ckeditor/_source`;
